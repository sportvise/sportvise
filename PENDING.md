...
**Chantier images + PDF dans le chat (14/06 soir, TESTÉ OK sur deploy preview PR #101, prêt à merger) — `src/dashboard.html` + `netlify/functions/chat/index.js` + 4 i18n :**
...
- 🐛 Bug additionnel trouvé + corrigé pendant le test live : `requestBody.message` envoyait `msg` (vide quand seul un fichier est joint sans texte) → backend rejetait avec 400 "Missing agentId or message" (bug préexistant, pas introduit ce soir, aurait bloqué tout envoi fichier-seul même en prod). Fix : `message: fullMsg` (toujours non-vide avec un fichier joint — placeholder + tag `[📎 nom]`).
- Smoke test : 86/86 ✅. `node --check` sur `chat/index.js`, les 4 i18n, et les 11 blocs `<script>` de `dashboard.html` : OK.
- ✅ TESTÉ EN VRAI sur https://deploy-preview-101--prismatic-lebkuchen-48a8ee.netlify.app (14/06 soir) : (1) photo (bouteille de bière) envoyée à David → décrite correctement + conseil contextuel récupération, plus d'erreur de connexion ; (2) PDF planning hebdo FC Lausanne-Sport envoyé à David → contenu lu et restitué avec détails précis (dates/séances). Plus de 400 en console.
- Reste à faire : merger PR #101 (`test-image-pdf-chat`) dans `main` via GitHub web, bumper `src/version.json`.
...
**Bug brief matinal — push jamais envoyé (15/06, à investiguer en session dédiée) :**
- `push_subscriptions` : 7 souscriptions actives (créées entre 09/05 et 15/06), `last_pushed_at` = NULL pour les 7, `fail_count: 0`.
- `api_usage_log` filtré `endpoint=morning_brief` : 0 ligne, jamais. La fonction `send-morning-brief` (déclenchée par cron-job.org chaque matin 7h) n'a donc jamais été appelée.
- À vérifier : (1) cron-job.org configuré et actif ? (2) `BRIEF_TRIGGER_TOKEN` bien défini dans Netlify env vars ? (3) si appel manuel direct de la fonction avec le bon token fonctionne.
- Impact : 7 users opt-in (dont comptes test) ne reçoivent rien depuis jusqu'à 5+ semaines — feature "killer #1" silencieusement morte.
...
- ~~Bug "Une erreur de connexion" sur envoi d'image lourde au chat~~ ✅ corrigé 14/06 (voir chantier images+PDF ci-dessus)
